...
| V1-FIX-003 | accepted | Tag assignment is unclear and currently attaches a global new tag to the first item. | `apps/web/src/App.vue`, `apps/web/test/App.test.ts`, `dev_management/v1_feedback_fixes.md`, `dev_management/action_log.md`, `docs/product/user-stories.md`, `docs/development/manual-smoke-scenarios.md`, `docs/release/windows-v1-checklist.md` | `npm run typecheck`, `npm run lint`, `npm test` passed on 2026-04-22. Manual QA accepted on 2026-04-22. | Item cards get scoped tag inputs with existing-tag suggestions; top `Create tag` only creates a global tag and attaches nothing. Bulk assignment remains deferred. |
